...
BATCH_STATUS_POLLING_SLEEP = 10
PK_CHUNKED_BATCH_STATUS_POLLING_SLEEP = 30
...
class Bulk:
    ...
    def _bulk_query(self, catalog_entry, state):
        ...
        LOGGER.info("Waiting on batches")
        ...
        if batch_status["state"] == "Failed":
            if "QUERY_TIMEOUT" in batch_status["stateMessage"]:
                ...
                for completed_batch_id in batch_status["completed"]:
                    ...
                    LOGGER.info(
                        "Finished syncing batch %s. Removing batch from state.", completed_batch_id
                    )
                    LOGGER.info(
                        "Batches to go: %d",
    ...
    def _create_job(self, catalog_entry, pk_chunking=False):
        ...
        LOGGER.info("Creating job {}".format(catalog_entry["stream"]))
    ...
    def _poll_on_batch_status(self, job_id, batch_id):
        ...
        while batch_status["state"] not in ["Completed", "Failed", "Not Processed"]:
            ...
            wait_count += 1
            if wait_count % 5 == 0:
                LOGGER.info("Waited {} seconds".format(wait_count * BATCH_STATUS_POLLING_SLEEP))
            batch_status = self._get_batch(job_id=job_id, batch_id=batch_id)
    ...
    def _get_batches(self, job_id):
        ...
        LOGGER.info("{} batches to fetch for {}".format(len(batches.get("result", [])), job_id))
        return batches
            ...
            with tempfile.NamedTemporaryFile(mode="w+", encoding="utf8") as csv_file:
                ...
                for line in csv_reader:
                    try:
                        ...
                        logger.error(f"Unable to proc {line}")
            counter += 1
